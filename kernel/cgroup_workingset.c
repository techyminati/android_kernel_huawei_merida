...
{
	struct s_pagecache_info *temp = NULL;
	...
	unsigned *repeat_pages, int *page_count_delta)
{
	...
	struct s_pagecache_info *myentry = NULL, *merged_entry = NULL;
...
{
	unsigned idx = 0;
	struct page *page = NULL, **page_array = NULL;
...
static void workingset_destroy_data(struct s_workingset *ws, bool is_locked)
{
	...
	struct s_filp_list *curr = NULL, *next = NULL;
	...
	struct file *file, struct s_file_info **file_info, bool *is_existed)
{
	...
	struct s_filp_list *filp_list = NULL;
	struct list_head *pos = NULL;
	struct list_head *head = NULL;
	unsigned int path_len;
	char *filepath = NULL;
	...
	struct s_workingset *ws)
{
	...
	struct s_pagecache_info *pagecache = NULL;
...
{
	int ret;
	struct s_ws_collector *collector = NULL;
...
{
	struct s_cachepage_info info, *target_space = NULL;
