...
static void musb_irq_notify_work(struct work_struct *work)
{
	if (!work)
		return;
	if (notify_flag)
		otg_notifier_call_chain(OTG_CHARGER_SLAVE_CONNECT_EVENT);
	else
		otg_notifier_call_chain(OTG_CHARGER_SAVLE_DISCONNECT_EVENT);
}
...
#endif
{
	...
	INIT_WORK(&notify_work, musb_irq_notify_work);
