...
static BLOCKING_NOTIFIER_HEAD(otg_notifier_head);
int otg_register_notifier(struct notifier_block *nb)
{
	if (!nb)
		return -EINVAL;
	return blocking_notifier_chain_register(&otg_notifier_head, nb);
}
EXPORT_SYMBOL_GPL(otg_register_notifier);

int otg_notifier_unregister(struct notifier_block *nb)
{
	if (!nb)
		return -EINVAL;
	return blocking_notifier_chain_unregister(&otg_notifier_head, nb);
}
EXPORT_SYMBOL_GPL(otg_notifier_unregister);

int otg_notifier_call_chain(unsigned long val)
{
	return (blocking_notifier_call_chain(&otg_notifier_head, val, NULL) ==
		NOTIFY_BAD) ? -EINVAL : 0;
}
...
#ifdef CONFIG_Y_CABLE
	ycable_reset_type();
	if (ycable_get_type() == Y_CABLE_CHARGER) {
		ycable_set_vbus_status();
		vbus_on = g_vbus_status;
		return;
	}
	ycable_reset_type();
#endif
...
#endif
#endif
		ret = otg_notifier_call_chain(OTG_CHARGER_CONNECT_EVENT);
	} else if (!is_on && vbus_on) {
		/* disable VBUS 1st then update flag
		 * to make host mode correct used by PMIC
		 */
		vbus_on = false;
		ret = otg_notifier_call_chain(OTG_CHARGER_DISCONNECT_EVENT);
...
void switch_int_to_device(struct musb *musb)
{
	...
	irq_set_irq_wake(iddig_eint_num, 1);
...
static int hw_otg_id_adc_sampling(void)
{
	...
	msleep(100); /* 100ms for adc stable */
	for (i = 0; i < HW_OTG_MAX_COUNT; i++) {
		ret = IMM_GetOneChannelValue_Cali(ID_ADC_CHANNEL, &vol_value);
		msleep(SAMPLING_TIME_INTERVAL);
		if (ret != 0) {
			DBG(0, "%s find otg_iddig node fail\n", __func__);
			continue;
		}
		sum += vol_value;
		sample_cnt++;
...
static void do_host_work(struct work_struct *data)
{
	...
	if (work->ops == CONNECTION_OPS_CONN) {
		...
		if ((avgvalue < 0) || (avgvalue > ADC_VOLTAGE_LIMIT) ||
			vbus_exist) {
#ifdef CONFIG_Y_CABLE
			if  ((avgvalue > CHARGER_VOLTAGE_LOW_THRESHOLD) &&
				(avgvalue < CHARGER_VOLTAGE_HIGH_THRESHOLD)) {
				DBG(0, "y cable charger and otg\n");
			} else {
#endif
				DBG(0, "%s adc=%d, vbus=%d\n",
					__func__, avgvalue, vbus_exist);
				g_abnormal_sign = true;
				irq_set_irq_type(iddig_eint_num, IRQF_TRIGGER_HIGH);
				enable_irq(iddig_eint_num);
				return;
#ifdef CONFIG_Y_CABLE
	...
	} else if ((g_abnormal_sign == true) &&
		(work->ops == CONNECTION_OPS_DISC)) {
		DBG(0, "%s abnormal insert, set irq low\n", __func__);
		g_abnormal_sign = false;
		irq_set_irq_type(iddig_eint_num, IRQF_TRIGGER_LOW);
		enable_irq(iddig_eint_num);
		enable_irq_wake(iddig_eint_num);
		return;
