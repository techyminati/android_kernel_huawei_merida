...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		if (strncmp(CONFIG_ARCH_MTK_PROJECT, "tb8768p1", 8) == 0) {
			extern int camcal_read_insensor_hi556_func(unsigned char *eeprom_buff,
							int eeprom_size);

			extern int camcal_read_insensor_gc5035p_func(unsigned char *eeprom_buff,
							int eeprom_size);

			if (HI556_SENSOR_ID == ptempbuf->sensorID) {
				i4RetValue = camcal_read_insensor_hi556_func(pu1Params, ptempbuf->u4Length);
				if (i4RetValue < 0) {
					pr_err("hi556 read otp data error!\n");
					kfree(pBuff);
					kfree(pu1Params);
					return -EFAULT;
				}
			...
			} else if (GC5035P_MIPI_SENSOR_ID == ptempbuf->sensorID) {
				i4RetValue = camcal_read_insensor_gc5035p_func(pu1Params, ptempbuf->u4Length);
				if (i4RetValue < 0) {
					pr_err("gc5035 read otp data error!\n");
					kfree(pBuff);
					kfree(pu1Params);
					return -EFAULT;
				}
		...
		i4RetValue = eeprom_hal_get_data(pu1Params, ptempbuf->sensorID,
			ptempbuf->u4Length);
		if (i4RetValue < 0)
		{
