...
struct pinctrl *g_pinctrl3;
struct pinctrl_state *g_red_en_edfault;
struct pinctrl_state *g_red_en_low;
struct pinctrl_state *g_red_en_high;

int leds_red_ctrl_set(int en)
{
	int ret = -1;
	if (g_pinctrl3 && g_red_en_high && g_red_en_low) {
		if(en)
			ret = pinctrl_select_state(g_pinctrl3, g_red_en_high);
		else
			ret = pinctrl_select_state(g_pinctrl3, g_red_en_low);
		return ret;
	}
...
{
	int ret = 0;
	g_pinctrl3 = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(g_pinctrl3)) {
		ret = PTR_ERR(g_pinctrl3);
		LEDS_DRV_DEBUG ("Cannot find g_pinctrl3\n");
		return ret;
	}
	g_red_en_edfault = pinctrl_lookup_state(g_pinctrl3, "g_redEnDefault");
	if (IS_ERR(g_red_en_edfault)) {
		ret = PTR_ERR(g_red_en_edfault);
		LEDS_DRV_DEBUG ("Cannot find pinctrl default\n");
	}
	g_red_en_low = pinctrl_lookup_state(g_pinctrl3, "g_redEnLow");
	if (IS_ERR(g_red_en_low)) {
		ret = PTR_ERR(g_red_en_low);
		LEDS_DRV_DEBUG ("Cannot find pinctrl chg_en_low\n");
		return ret;
	}
	g_red_en_high = pinctrl_lookup_state(g_pinctrl3, "g_redEnHigh");
	if (IS_ERR(g_red_en_high)) {
		ret = PTR_ERR(g_red_en_high);
	...
	pmic_set_register_value(PMIC_CHRIND_DIM_FSEL, ISINK_128K_500HZ);
...
static int leds_red_ctrl_remov(struct platform_device *pdev)
{
	return 0;
}

const struct of_device_id g_leds_red_ctrl_of_match[] = {
	{
...
static struct platform_driver g_leds_red_ctrl_driver = {
	.driver = {
		   .name = "leds_red_ctrl",
		   .owner = THIS_MODULE,
		   .of_match_table = of_match_ptr(g_leds_red_ctrl_of_match),
		   },
	.probe = leds_red_ctrl_probe,
	.remove = leds_red_ctrl_remov,
	.shutdown = NULL,
};
...
static int __init mt65xx_leds_init(void)
{
	...
	ret = platform_driver_register(&g_leds_red_ctrl_driver);
	if (ret) {
		pr_info("[LED]register red ctrl\n");
	}
