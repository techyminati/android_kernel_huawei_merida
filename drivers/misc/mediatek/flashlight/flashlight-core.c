...
static int g_factory_flash_state[MAX_FLASH_NUM] = {0,0};
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (mode == STANDBY_MODE) {
		if (g_factory_flash_state[mtk_flash_id - 1] == STANDBY_MODE) {
		...
		g_factory_flash_state[mtk_flash_id - 1] = STANDBY_MODE;
	} else if(mode >= TORCH_MODE) {
		struct flashlight_dev_arg fl_dev_arg;
		if (g_factory_flash_state[mtk_flash_id - 1] == TORCH_MODE) {
		...
		if (fdev->dev_id.decouple) {
			fl_dev_arg.arg = FLASHLIGHT_SCENARIO_DECOUPLE;
			rc |= fdev->ops->flashlight_ioctl(FLASH_IOC_SET_SCENARIO,
				(unsigned long)&fl_dev_arg);
		}
		fl_dev_arg.arg = 0;
		rc |= fl_set_level(fdev, 0); // 100ma
		rc |= fdev->ops->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS,
			(unsigned long)&fl_dev_arg);
		mutex_lock(&fl_mutex);
		rc |= fl_enable(fdev, 1);
		mutex_unlock(&fl_mutex);
		...
		g_factory_flash_state[mtk_flash_id - 1] = TORCH_MODE;
	} else {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&fl_mutex);
	if (op_code == FLASH_STATUS_UNLOCK) {
...
static struct device_attribute hw_flash_thermal_protect =
	__ATTR(flash_thermal_protect, 0660, hw_flash_thermal_protect_show,
