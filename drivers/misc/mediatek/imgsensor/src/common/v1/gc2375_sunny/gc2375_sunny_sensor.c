/*
 * gc2375_sunny_sensor.c
 *
 * gc2375_sunny image sensor driver
 *
...
#define PFX "[gc2375_sunny]"
#define DEBUG_GC2375_SUNNY 1
#define LOG_DBG(fmt, args...) \
	do { \
		if (DEBUG_GC2375_SUNNY) \
			pr_debug(PFX "%s %d " \
...
static kal_uint32 set_hs_video_setting(kal_uint16 currefps)
{
	kal_int32 rc = 0;
	LOG_DBG("ENTER\n");
	rc = imgsensor_sensor_write_setting(&imgsensor, &imgsensor_info.hs_video_setting);
	if (rc < 0) {
		LOG_ERR("Failed.\n");
		return ERROR_DRIVER_INIT_FAIL;
	}
...
static kal_uint32 set_ls_video_setting(kal_uint16 currefps)
{
	kal_int32 rc = 0;
	LOG_DBG("ENTER\n");
	rc = imgsensor_sensor_write_setting(&imgsensor, &imgsensor_info.slim_setting);
	if (rc < 0) {
		LOG_ERR("Failed.\n");
		return ERROR_DRIVER_INIT_FAIL;
	}
...
static kal_uint32 feature_control_gc2375_sunny(MSDK_SENSOR_FEATURE_ENUM feature_id,
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	get_info,
	get_resolution,
	feature_control_gc2375_sunny,
	control,
	close
};

UINT32 GC2375_SUNNY_SensorInit(struct SENSOR_FUNCTION_STRUCT **pf_func)
{
