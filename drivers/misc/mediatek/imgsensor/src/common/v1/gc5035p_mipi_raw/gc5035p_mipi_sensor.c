...
#include "gc5035p_mipi_sensor.h"

#define PFX "gc5035p_camera_sensor"
#define LOG_1 log_inf("GC5035P_MIPI, 2LANE\n")
#define GC5035P_DEBUG 0
...
static kal_uint32 dgain_ratio = GC5035P_SENSOR_DGAIN_BASE;
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5035P_MIPI_SENSOR_ID,
...
static kal_uint16 gc5035p_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len) {
			    ...
			    int eeprom_size)
{
	...
	memcpy_s(eeprom_buff, eeprom_size, gc5035p_otp_data.otp_buf, eeprom_size);
...
static kal_uint8 gc5035p_otp_read_byte(kal_uint16 addr)
{
...
static void gc5035p_otp_read_group(kal_uint16 addr,
...
static void gc5035p_gcore_read_dpc(void)
{
	kal_uint8 dpcflag = 0;
	struct gc5035p_dpc_t *pdpc = &gc5035p_otp_data.dpc;

	dpcflag = gc5035p_otp_read_byte(GC5035P_OTP_DPC_FLAG_OFFSET);
...
static void gc5035p_gcore_read_reg(void)
{
	kal_uint8 i = 0;
	kal_uint8 j = 0;
	kal_uint16 base_group = 0;
	kal_uint8 reg[GC5035P_OTP_REG_DATA_SIZE];
	...
	pregs->flag = gc5035p_otp_read_byte(GC5035P_OTP_REG_FLAG_OFFSET);
	log_inf("register update flag = 0x%x\n", pregs->flag);
	if (pregs->flag == GC5035P_OTP_FLAG_VALID) {
		gc5035p_otp_read_group(GC5035P_OTP_REG_DATA_OFFSET,
		...
		for (i = 0; i < GC5035P_OTP_REG_MAX_GROUP; i++) {
			...
			for (j = 0; j < GC5035P_OTP_REG_REG_PER_GROUP; j++)
				if (gc5035p_otp_check_1bit_flag(
...
static void gc5035p_otp_read_sn_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 flag = 0;
	kal_uint16 check = 0;
	kal_uint16 sn_start_offset = 0;
	kal_uint8 info[GC5035P_OTP_SN_DATA_SIZE];

	memset_s(&info, GC5035P_OTP_SN_DATA_SIZE, 0, GC5035P_OTP_SN_DATA_SIZE);
	flag = gc5035p_otp_read_byte(GC5035P_OTP_SN_FLAG_OFFSET);
	...
	gc5035p_otp_read_group(sn_start_offset, &info[0], GC5035P_OTP_SN_DATA_SIZE);
...
static void gc5035p_otp_read_af_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 flag = 0;
	kal_uint16 check = 0;
	kal_uint16 af_start_offset = 0;
	kal_uint8 info[GC5035P_OTP_AF_DATA_SIZE];

	memset_s(&info, GC5035P_OTP_AF_DATA_SIZE, 0, GC5035P_OTP_AF_DATA_SIZE);
	flag = gc5035p_otp_read_byte(GC5035P_OTP_AF_FLAG_OFFSET);
	if ((flag & 0x03) == 0x01) {
	...
	gc5035p_otp_read_group(af_start_offset, &info[0], GC5035P_OTP_AF_DATA_SIZE);
	...
	if ((check % 255 + 1) == info[GC5035P_OTP_AF_DATA_SIZE - 1]) { /* 255: checksum base value  */
		...
		gc5035p_otp_data.otp_buf[1] = info[1];
		gc5035p_otp_data.otp_buf[2] = info[2];
		gc5035p_otp_data.otp_buf[3] = info[3];
		gc5035p_otp_data.otp_buf[4] = info[4];
...
static kal_uint8 gc5035p_otp_read_module_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 flag = 0;
	kal_uint16 check = 0;
	kal_uint16 module_start_offset = 0;
	kal_uint8 info[GC5035P_OTP_MODULE_DATA_SIZE];
	struct gc5035p_module_info_t module_info = { 0 };

	memset_s(&info, GC5035P_OTP_MODULE_DATA_SIZE, 0, GC5035P_OTP_MODULE_DATA_SIZE);
	memset_s(&module_info, sizeof(struct gc5035p_module_info_t), 0, sizeof(struct gc5035p_module_info_t));
	flag = gc5035p_otp_read_byte(GC5035P_OTP_MODULE_FLAG_OFFSET);
	...
	gc5035p_otp_read_group(module_start_offset, &info[0], GC5035P_OTP_MODULE_DATA_SIZE);
...
static void gc5035p_otp_read_gwb_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 flag = 0;
	kal_uint16 golden_check = 0;
	kal_uint16 golden_start_offset = 0;
	kal_uint8 golden[GC5035P_OTP_GOLDEN_DATA_SIZE];
	struct gc5035p_wb_t *pgolden = &gc5035p_otp_data.golden;

	memset_s(&golden, GC5035P_OTP_GOLDEN_DATA_SIZE, 0, GC5035P_OTP_GOLDEN_DATA_SIZE);
	flag = gc5035p_otp_read_byte(GC5035P_OTP_WB_FLAG_OFFSET);
	...
	/* Golden AWB */
	gc5035p_otp_read_group(golden_start_offset, &golden[0], GC5035P_OTP_GOLDEN_DATA_SIZE);
...
static void gc5035p_otp_read_wb_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 flag = 0;
	kal_uint16 wb_check = 0;
	kal_uint16 wb_start_offset = 0;
	kal_uint8 wb[GC5035P_OTP_WB_DATA_SIZE];
	struct gc5035p_wb_t *pwb = &gc5035p_otp_data.wb;

	memset_s(&wb, GC5035P_OTP_WB_DATA_SIZE, 0, GC5035P_OTP_WB_DATA_SIZE);
	flag = gc5035p_otp_read_byte(GC5035P_OTP_WB_FLAG_OFFSET);
	...
	/* AWB */
	gc5035p_otp_read_group(wb_start_offset, &wb[0], GC5035P_OTP_WB_DATA_SIZE);
...
static kal_uint8 gc5035p_otp_read_sensor_info(void)
{
	kal_uint8 moduleid = 0;
#if GC5035P_OTP_DEBUG
	kal_uint16 i = 0;
	kal_uint8 debug[GC5035P_OTP_DATA_LENGTH];
#endif

	gc5035p_gcore_read_dpc();
	gc5035p_gcore_read_reg();

	moduleid = gc5035p_otp_read_module_info();
	gc5035p_otp_read_wb_info();
	gc5035p_otp_read_gwb_info();
	gc5035p_otp_read_sn_info();
	gc5035p_otp_read_af_info();

#if GC5035P_OTP_DEBUG
	memset_s(&debug[0], GC5035P_OTP_DATA_LENGTH, 0, GC5035P_OTP_DATA_LENGTH);
	gc5035p_otp_read_group(GC5035P_OTP_START_ADDR, &debug[0], GC5035P_OTP_DATA_LENGTH);
...
static void gc5035p_otp_update_dd(void)
{
	kal_uint8 state = 0;
	kal_uint8 n = 0;
	struct gc5035p_dpc_t *pdpc = &gc5035p_otp_data.dpc;
...
static void gc5035p_otp_update_wb(void)
{
	kal_uint16 r_gain = GC5035P_OTP_WB_GAIN_BASE;
	kal_uint16 g_gain = GC5035P_OTP_WB_GAIN_BASE;
	kal_uint16 b_gain = GC5035P_OTP_WB_GAIN_BASE;
	kal_uint16 base_gain = GC5035P_OTP_WB_CAL_BASE;
	kal_uint16 r_gain_curr = GC5035P_OTP_WB_CAL_BASE;
	kal_uint16 g_gain_curr = GC5035P_OTP_WB_CAL_BASE;
	kal_uint16 b_gain_curr = GC5035P_OTP_WB_CAL_BASE;
	kal_uint16 rg_typical = GC5035P_OTP_WB_RG_TYPICAL;
	kal_uint16 bg_typical = GC5035P_OTP_WB_BG_TYPICAL;
	struct gc5035p_wb_t *pwb = &gc5035p_otp_data.wb;
	struct gc5035p_wb_t *pgolden = &gc5035p_otp_data.golden;
...
static void gc5035p_otp_update_reg(void)
{
	kal_uint8 i = 0;
...
static void gc5035p_otp_update(void)
{
	gc5035p_otp_update_dd();
	gc5035p_otp_update_wb();
	gc5035p_otp_update_reg();
}

static kal_uint8 gc5035p_otp_identify(void)
{
	kal_uint8 moduleid = 0;

	memset_s(&gc5035p_otp_data, sizeof(struct gc5035p_otp_t), 0, sizeof(gc5035p_otp_data));
	...
	gc5035p_otp_read_group(
	GC5035P_OTP_ID_DATA_OFFSET,
	&gc5035p_otp_data.otp_id[0],
	GC5035P_OTP_ID_SIZE);
	moduleid = gc5035p_otp_read_sensor_info();
...
static void gc5035p_otp_function(void)
{
	kal_uint8 i = 0, flag = 0;
	kal_uint8 otp_id[GC5035P_OTP_ID_SIZE];

	memset_s(&otp_id, GC5035P_OTP_ID_SIZE, 0, GC5035P_OTP_ID_SIZE);
	...
	gc5035p_otp_read_group(GC5035P_OTP_ID_DATA_OFFSET,
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < GC5035P_SENSOR_GAIN_BASE)
		reg_gain = GC5035P_SENSOR_GAIN_BASE;
	else if (reg_gain > GC5035P_SENSOR_GAIN_MAX)
		reg_gain = GC5035P_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 gc5035p_agc_param[GC5035P_SENSOR_GAIN_MAP_SIZE][2] = {
		{  256,  0 },
		{  302,  1 },
		{  358,  2 },
		{  425,  3 },
	...
	for (gain_index = GC5035P_SENSOR_GAIN_MAX_VALID_INDEX - 1;
	gain_index >= 0; gain_index--)
		if (reg_gain >= gc5035p_agc_param[gain_index][0])
			break;

	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xb6, gc5035p_agc_param[gain_index][1]);
	temp_gain = reg_gain * dgain_ratio / gc5035p_agc_param[gain_index][0];
	write_cmos_sensor(0xb1, (temp_gain >> 8) & 0x0f);
	write_cmos_sensor(0xb2, temp_gain & 0xfc);
	log_inf("Exit! gc5035p_agc_param[gain_index][1] = 0x%x",
		gc5035p_agc_param[gain_index][1]);
...
#ifdef MULTI_WRITE
kal_uint16 addr_data_pair_init_gc5035p[] = {
	0xfc, 0x01,
	0xf4, 0x40,
	0xf5, 0xe9,
	0xf6, 0x14,
...
static void sensor_init(void)
{
	log_inf("E\n");
#ifdef MULTI_WRITE
	gc5035p_table_write_cmos_sensor(
		addr_data_pair_init_gc5035p,
		sizeof(addr_data_pair_init_gc5035p) /
		sizeof(kal_uint16));
#else
	/* SYSTEM */
	write_cmos_sensor(0xfc, 0x01);
	...
	write_cmos_sensor(0x17, GC5035P_MIRROR);
	...
	write_cmos_sensor(0x54, GC5035P_RSTDUMMY1);
	...
	write_cmos_sensor(0x22, GC5035P_RSTDUMMY2);
...
#ifdef MULTI_WRITE
kal_uint16 addr_data_pair_preview_gc5035p[] = {
	0xfc, 0x01,
	0xf4, 0x40,
	0xf5, 0xe4,
	0xf6, 0x14,
...
#ifdef MULTI_WRITE
	gc5035p_table_write_cmos_sensor(
		addr_data_pair_preview_gc5035p,
		sizeof(addr_data_pair_preview_gc5035p) /
		sizeof(kal_uint16));
#else
	/* System */
	write_cmos_sensor(0xfc, 0x01);
...
#ifdef MULTI_WRITE
kal_uint16 addr_data_pair_capture_24fps_gc5035p[] = {
	0xfc, 0x01,
	0xf4, 0x40,
	0xf5, 0xe7,
	0xf6, 0x14,
...
kal_uint16 addr_data_pair_capture_fps_gc5035p[] = {
	0xfc, 0x01,
	0xf4, 0x40,
	0xf5, 0xe9,
	0xf6, 0x14,
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 240)
		gc5035p_table_write_cmos_sensor(
			addr_data_pair_capture_24fps_gc5035p,
			sizeof(addr_data_pair_capture_24fps_gc5035p) /
			sizeof(kal_uint16));
	else
		gc5035p_table_write_cmos_sensor(
			addr_data_pair_capture_fps_gc5035p,
			sizeof(addr_data_pair_capture_fps_gc5035p) /
			sizeof(kal_uint16));
#else
	/* System */
	write_cmos_sensor(0xfc, 0x01);
...
#ifdef MULTI_WRITE
kal_uint16 addr_data_pair_normal_video_gc5035p[] = {
	0xfc, 0x01,
	0xf4, 0x40,
	0xf5, 0xe4,
	0xf6, 0x14,
...
#ifdef MULTI_WRITE
	gc5035p_table_write_cmos_sensor(
		addr_data_pair_normal_video_gc5035p,
		sizeof(addr_data_pair_normal_video_gc5035p) /
		sizeof(kal_uint16));
#else
	/* System */
	write_cmos_sensor(0xfc, 0x01);
...
#ifdef MULTI_WRITE
kal_uint16 addr_data_pair_hs_video_gc5035p[] = {
	0xfc, 0x01,
	0xf4, 0x40,
	0xf5, 0xe9,
	0xf6, 0x14,
...
#ifdef MULTI_WRITE
	gc5035p_table_write_cmos_sensor(
		addr_data_pair_hs_video_gc5035p,
		sizeof(addr_data_pair_hs_video_gc5035p) /
		sizeof(kal_uint16));
#else
	/* System */
	write_cmos_sensor(0xfc, 0x01);
...
#ifdef MULTI_WRITE
kal_uint16 addr_data_pair_slim_video_gc5035p[] = {
	0xfc, 0x01,
	0xf4, 0x40,
	0xf5, 0xe4,
	0xf6, 0x14,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			log_inf("[gc5035p_camera_sensor]\n");
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				log_inf("[gc5035p_camera_sensor]\n");
				#if GC5035P_OTP_FOR_CUSTOMER
				gc5035p_otp_identify();
				#endif
				log_inf("[gc5035p]%s:i2c write id: 0x%x",
				__func__, imgsensor.i2c_write_id);
				log_inf("[gc5035p]%s:sensor id: 0x%x\n",
				__func__, *sensor_id);
				return ERROR_NONE;
			}
			log_err("[gc5035p]%s:fail, write id: 0x%x",
			__func__, imgsensor.i2c_write_id);
			log_err("[gc5035p]%s:fail,id: 0x%x\n",
			__func__, *sensor_id);
			retry--;
			mdelay(1000);
		} while (retry > 0);
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		log_err("[gc5035p] *sensor_id = 0xFFFFFFFFr");
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_debug("[gc5035p]%s:i2c write id: 0x%x",
				__func__, imgsensor.i2c_write_id);
				pr_debug("[gc5035p]%s:sensor id: 0x%x\n",
				__func__, sensor_id);
				break;
			}
			log_err("[gc5035p]%s:fail, write id: 0x%x",
...
UINT32 GC5035P_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pffunc)
{
