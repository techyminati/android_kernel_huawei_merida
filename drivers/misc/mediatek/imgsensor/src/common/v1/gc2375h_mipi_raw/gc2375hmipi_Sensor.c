...
#define DATA_LINE_LENGTH 1224
#define MAX_VB_LINE_LENGTH 8191
#define MIN_VB_LINE_LENGTH 16
...
static kal_uint16 gc2375h_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len) {
...
{
	kal_uint32 vb = 0;
	kal_uint32 basic_line = DATA_LINE_LENGTH;

	cam_pr_debug("dummyline = %d\n", imgsensor.dummy_line);
	if(imgsensor.frame_length < basic_line)
		return;

	vb = imgsensor.frame_length - basic_line;
	vb = (vb < MIN_VB_LINE_LENGTH) ? MIN_VB_LINE_LENGTH : vb;
	vb = (vb > MAX_VB_LINE_LENGTH) ? MAX_VB_LINE_LENGTH : vb;
...
kal_uint16 pair_init_gc2375h[] = {
	...
	0x43, 0x03,
	0x4a, 0x00,
	0x4e, GC2375H_BLK_Select1_H,
	0x4f, GC2375H_BLK_Select1_L,
	0x66, GC2375H_BLK_Select2_H,
	0x67, GC2375H_BLK_Select2_L,
	0x68, 0x00,
	0xb0, 0x58,
	0xb1, 0x01,
	0xb2, 0x00,
...
#ifdef MULTI_WRITE
	gc2375h_table_write_cmos_sensor(
		pair_init_gc2375h,
		sizeof(pair_init_gc2375h) /
		sizeof(kal_uint16));
#else
	/* System */
	write_cmos_sensor(0xfe, 0x00);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			cam_pr_err("Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	case MSDK_SCENARIO_ID_CUSTOM2:
		...
		break;
	default:
		cam_pr_err("Error ScenarioId setting");
		preview(image_window, sensor_config_data);
	...
	MUINT32 *framerate)
{
	cam_pr_debug("scenario_id = %d\n", scenario_id);
	if (framerate == NULL) {
		cam_pr_err("---- framerate is NULL ---- check here\n");
		return ERROR_NONE;
	}
	spin_lock(&imgsensor_drv_lock);
	switch (scenario_id) {
