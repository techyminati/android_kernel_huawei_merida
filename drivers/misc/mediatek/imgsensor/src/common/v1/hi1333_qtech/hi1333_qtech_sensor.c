 ...
 *
 * hi1333_qtech image sensor driver
 *
...
#define Hi1333_MAXGAIN 16
#define REG_GAIN_1X 16
#define REG_GAIN_OFFSET 16
#define LINE_LENGTH_TIMES 8 /* configured / actual */
...
static void set_dummy(void)
{
	...
	rc = imgsensor_sensor_i2c_write(&imgsensor,
			...
			imgsensor.line_length / LINE_LENGTH_TIMES,
			IMGSENSOR_I2C_WORD_DATA);
	if (rc < 0) {
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	/* sensor spec agin = reg_gain/16 + 1,so reg_gain like this */
	reg_gain = (gain * REG_GAIN_1X) / BASEGAIN - REG_GAIN_OFFSET;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain < BASEGAIN) {
		LOG_ERR("Invaild gain: %u\n", gain);
		gain = BASEGAIN;
	} else if (gain > Hi1333_MAXGAIN * BASEGAIN) {
		LOG_ERR("Invaild gain: %u\n", gain);
		gain = Hi1333_MAXGAIN * BASEGAIN;
	}
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	 ...
	 * bit[15~12]: reserve
	 * bit[11:8]:
	 * 0:no pattern, 1:solid color,2:100% color bar,
	 * 3:fade to grey color bar,4:pn9,5:H gradient pattern
	 * 7:check board, 8:slant pattern, 9:resolution pattern,
	 * 10~255:reserved
	 * bit[7:0]: reserved
	 */
	if (enable) {
		(void)imgsensor_sensor_i2c_write(&imgsensor,
			...
			IMGSENSOR_I2C_WORD_DATA);
		(void)imgsensor_sensor_i2c_write(&imgsensor,
			SENSOR_TEST_PATTERN_REG, 0x0200,
			IMGSENSOR_I2C_WORD_DATA);

	} else {
		(void)imgsensor_sensor_i2c_write(&imgsensor,
			SENSOR_ISP_EN_REG_H, 0x03C1,
			IMGSENSOR_I2C_WORD_DATA);
		(void)imgsensor_sensor_i2c_write(&imgsensor,
			SENSOR_TEST_PATTERN_REG, 0x0000,
			IMGSENSOR_I2C_WORD_DATA);
	}
	spin_lock(&imgsensor_drv_lock);
...
static void hi1333_qtech_get_crop_info(UINT32 feature,
...
static void hi1333_qtech_get_mipi_pixel_rate(unsigned long long *feature_data)
{
	switch (*feature_data) {
	case MSDK_SCENARIO_ID_CUSTOM1:
...
static void hi1333_qtech_get_pixel_rate(unsigned long long *feature_data)
{
	switch (*feature_data) {
	case MSDK_SCENARIO_ID_CUSTOM1:
...
static void hi1333_qtech_get_pdaf_info(struct SET_PD_BLOCK_INFO_T *PDAFinfo,
...
static void hi1333_qtech_get_pdaf_capacity(unsigned long long *feature_data)
{
	switch (*feature_data) {
	case MSDK_SCENARIO_ID_CUSTOM1:
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
...
static kal_uint32 feature_control_hi1333_qtech(MSDK_SENSOR_FEATURE_ENUM
	feature_id,
	UINT8 *feature_para,
	UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		...
		hi1333_qtech_get_crop_info(*feature_data_32, wininfo);
		break;
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		LOG_INF("GET_PDAF_INFO scenarioId:%u\n",
			(UINT32)*feature_data);
		PDAFinfo = (struct SET_PD_BLOCK_INFO_T *)
			(uintptr_t)(*(feature_data+1));
		hi1333_qtech_get_pdaf_info(PDAFinfo,feature_data);
		break;
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		LOG_INF("GET_SENSOR_PDAF_CAPACITY scenarioId:%u\n",
			(UINT32)*feature_data);
		hi1333_qtech_get_pdaf_capacity(feature_data);
		break;
	case SENSOR_HUAWEI_FEATURE_DUMP_REG:
		sensor_dump_reg();
		break;
	case SENSOR_FEATURE_GET_MIPI_PIXEL_RATE:
		hi1333_qtech_get_mipi_pixel_rate(feature_data);
		break;
	case SENSOR_FEATURE_GET_PIXEL_RATE:
		hi1333_qtech_get_pixel_rate(feature_data);
		break;
	default:
