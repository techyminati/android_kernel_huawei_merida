/*
 * ov13855_ofilm_tdk_sensor.c
 *
 * Copyright (c) 2018-2019 Huawei Technologies Co., Ltd.
 *
 * ov13855_ofilm_tdk image sensor driver
 *
...
static void set_shutter_common(UINT32 shutter)
{
	...
	if (long_expo_time <= NORMAL_EXPOSE_TIME_MAX) {
		...
		imgsensor_sensor_write_setting(&imgsensor,
					&tdk_normal_to_long_ready_settings);
		...
		(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_INTEG_TIME_H,
			...
			IMGSENSOR_I2C_WORD_DATA);
		imgsensor_sensor_write_setting(&imgsensor,
					&tdk_normal_to_long_end_settings);
	}
...
UINT32 OV13855_OFILM_TDK_SensorInit(struct SENSOR_FUNCTION_STRUCT **pf_func)
{
	if (pf_func != NULL)
		*pf_func = &sensor_func;
	return ERROR_NONE;
