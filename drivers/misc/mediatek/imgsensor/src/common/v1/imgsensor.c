...
static struct dsm_client *g_imgsensor_dmd_client;
...
MUINT32 get_driver_name(MUINT32 sensor_id, unsigned char *drv_name, MUINT32 len)
{
	...
	for (j = 0; j < MAX_NUM_OF_SUPPORT_SENSOR; j++) {
		if (p_sensor_list[j].id == sensor_id) {
			ret = snprintf_s(drv_name, len,
				strlen(p_sensor_list[j].name),
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_OPEN:
		...
		if (!ret) {
			...
			char buf[DSM_BUFF_SIZE] = {0};
			snprintf(buf, DSM_BUFF_SIZE,
				 "imgsensor(%s) open fail,index(%d) state(%d)\n",
				 psensor->inst.psensor_name, psensor->inst.sensor_idx,
				 psensor->inst.state);
