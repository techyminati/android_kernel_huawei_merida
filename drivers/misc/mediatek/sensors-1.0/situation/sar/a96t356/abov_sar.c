...
static int abov_sar_open_report_data(int open)
{
	...
	if (open == 1) {
		write_register(abov_sar_ptr,
			ABOV_CTRL_MODE_REG, ABOV_NOMAL_MODE);
		if (is_first_open) {
		...
		write_register(abov_sar_ptr,
			ABOV_CTRL_MODE_REG, ABOV_STOP_MODE);
	}
	return 0;
}
	...
	const struct i2c_device_id *id)
{
	...
	ppdata = kzalloc(sizeof(ppdata), GFP_KERNEL);
	if (ppdata == NULL)
		return -ENOMEM;
	...
	ret = write_register(ppdev, ABOV_CTRL_MODE_REG, ABOV_STOP_MODE);
	if (ret < 0) {
