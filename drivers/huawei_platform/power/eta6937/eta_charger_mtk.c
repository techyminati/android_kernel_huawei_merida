...
static struct charger_device *g_charger_dev;
struct hrtimer g_kick_wdt_timer;
wait_queue_head_t g_wait_otg_wdt_que;
bool g_otg_wdt_timeout;
...
static int eta_enable_charger(struct eta6937 *bq)
{
	...
	if (bq->part_no == ETA6937) {
		...
		gpio_set_value(GPIO_CHG_EN_0, GPIO_OUT_LOW);
	}
...
static int eta_disable_charger(struct eta6937 *bq)
{
	...
	if (bq->part_no == ETA6937)
		...
		gpio_set_value(GPIO_CHG_EN_0, GPIO_OUT_HIGH);
...
static int eta_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	if (bq->part_no == SY6923) {
		if (en) {
			...
			hrtimer_cancel(&g_kick_wdt_timer);
		}
...
{
	hrtimer_init(&g_kick_wdt_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	g_kick_wdt_timer.function = otg_hrtimer_func;
}
...
static void otg_start_timer(void)
{
	...
	hrtimer_start(&g_kick_wdt_timer, ktime, HRTIMER_MODE_REL);
}
...
static int otg_kick_wdt_thread(void *arg)
{
	while (1) {
		...
		(void)eta_reset_watchdog_timer(g_charger_dev);
		otg_start_timer();
	}
	return SUCCESS;
}
