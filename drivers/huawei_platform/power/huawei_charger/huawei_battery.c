...
static struct charger_device *hw_get_charger_device(
	...
	struct charger_consumer *consumer, int idx, int input_current)
{
	...
	pr_info("%s:temp:%d input_current %d\n", __func__, temp, input_current);
	...
	if (temp < LOW_BATT_TEMP) {
		g_input_current = info->icl_current * MV_TO_UV;
		pr_info("DOUBLE_85,low temp set ibus:%d\n", g_input_current);
	} else {
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_CURRENT_NOW:
		val->intval = (battery_get_bat_current() / BAT_DIVI);
		break;
