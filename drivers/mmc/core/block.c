...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
			if (mmc_card_sd(card))
				dsm_sdcard_report(DSM_SDCARD_STATUS_BLK_STUCK_IN_PRG_ERR,
					DSM_SDCARD_BLK_STUCK_IN_PRG_ERR);
#endif
#ifdef CONFIG_HW_SD_HEALTH_DETECT
			 ...
			 int type)
{
	...
	if (mmc_card_mmc(host->card))
		DSM_EMMC_LOG(host->card, DSM_EMMC_RW_TIMEOUT_ERR,
			"%s:eMMC enter reset, type=%d, partition=%d\n", __func__,
			type, md->part_type);
#endif
...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
				if (mmc_card_sd(card))
					dsm_sdcard_report(DSM_SDCARD_STATUS_MMC_BLK_ECC_ERROR,
						DSM_SDCARD_MMC_BLK_ECC_ERR);
#endif
...
#ifdef CONFIG_HUAWEI_RWLOG
		mmc_start_cmdq_request_rwlog(brq, req);
#endif
...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
			if (mmc_card_sd(card)) {
				dsm_sdcard_report(DSM_SDCARD_STATUS_MMC_BLK_ABORT,
					DSM_SDCARD_MMC_BLK_ABORT);
				pr_err("Err num: %d, %s\n",
					DSM_SDCARD_MMC_BLK_ABORT,
					"sdcard block abort err");
			}
#endif
			mmc_blk_rw_cmd_abort(mq, card, old_req, mq_rq);
			mmc_blk_rw_try_restart(mq, new_req, mqrq_cur);
...
#ifdef CONFIG_MTK_EMMC_CQ_SUPPORT
		trace_mmc_blk_rw_end(areq_active->mrq_que->cmd->opcode,
					areq_active->mrq_que->cmd->arg,
					areq_active->mrq_que->data);
#else
		trace_mmc_blk_rw_end(areq_active->mrq->cmd->opcode,
					areq_active->mrq->cmd->arg,
					areq_active->mrq->data);
#endif
#endif
...
#ifdef CONFIG_HUAWEI_SDCARD_DSM
	if (mmc_card_sd(card))
		dsm_sdcard_report(DSM_SDCARD_REPORT_UEVENT,
