...
unsigned int write_latency_ms;
unsigned int write_interval;

static ssize_t write_latency_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int bytes_written;
	bytes_written = 0;
	bytes_written += scnprintf(buf + bytes_written,
			PAGE_SIZE - bytes_written, "%u\n", write_latency_ms);
	return bytes_written;
}

static ssize_t write_latency_store(struct device *dev,
...
static DEVICE_ATTR(write_latency, 0644,
		write_latency_show,
		write_latency_store);

static ssize_t write_interval_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int bytes_written;
	bytes_written = 0;
...
static ssize_t write_interval_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	long value;
	...
	if (kstrtol(buf, 0, &value))
		return -EINVAL;
	write_interval = value;
	return count;
}

static DEVICE_ATTR(write_interval, 0644,
		write_interval_show,
		write_interval_store);
...
int mmc_run_queue_thread(void *data)
{
	...
	bool is_done = false;
	unsigned int latency_write;
	unsigned int interval_write;
	...
	device_create_file(&host->class_dev, &dev_attr_write_latency);
	device_create_file(&host->class_dev, &dev_attr_write_interval);
	...
	while (1) {
		...
		if (!atomic_read(&host->cq_rw)) {
			...
			&& dat_mrq->cmd->opcode != MMC_EXECUTE_READ_TASK);
				...
				latency_write = write_latency_ms;
				interval_write = write_interval;
				if (latency_write &&
					(dat_mrq->cmd->opcode ==
					MMC_EXECUTE_WRITE_TASK)) {
					if (interval_write &&
						(!(write_num++ %
						interval_write)))
						msleep(latency_write);
					else if (!interval_write)
						msleep(latency_write);
				}
				     ...
				     enum mmc_blk_status *ret_stat)
{
	...
	if (status == MMC_BLK_SUCCESS && areq) {
		...
		trace_mmc_blk_rw_start(mrq->cmd->opcode,
					mrq->cmd->arg,
					mrq->data);
#endif
		start_err =
