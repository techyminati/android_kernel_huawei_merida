	...
	struct iocache_transaction **trans)
{
	...
	while (1) {
		wait_event_interruptible_timeout(ioch->alloc_pages_wait,
				atomic_read(&ioch->tot_uppages) <=
				atomic_read(&ioch->iocache_page_hard_limit) - 1,
				msecs_to_jiffies(IOCACHE_REGISTER_TIMEOUT));
		if (atomic_read(&ioch->tot_uppages) <
			atomic_read(&ioch->iocache_page_hard_limit))
			break;
		pr_info("iocache: register page timeout && cur tot_page: %d\n",
				atomic_read(&ioch->tot_uppages));
	}
...
static void iocache_discard_endio(struct bio *bio)
{
	...
	spin_unlock_irqrestore(&ioch->erase_lock, flags);
	if (atomic_sub_return(di->nrpages, &ioch->inflight_discard) +
		IOCACHE_INFLIGHT_DISCARD_LIMIT <= INFLIGHT_DISCARD_MAX_PAGES)
		wake_up(&ioch->inflight_discard_wait);
...
static void iocache_issue_discard(struct iocache_device *const ioch)
{
	...
	head = idx = 0;
	do {
		if (atomic_read(&ioch->inflight_discard) +
			IOCACHE_INFLIGHT_DISCARD_LIMIT >
			INFLIGHT_DISCARD_MAX_PAGES) {
			up_read(&ioch->commit_rwsem);
			/* release read lock, new_transaction can be inserted */
			down_read(&ioch->commit_rwsem);
			idle = is_idle(ioch);
			...
			if (!list_is_singular(&ioch->transactions) ||
				reach_limit ||
				cur_jiffies - ioch->jiffies_interval >=
				ioch->threshold[idle]) {
				break;
			} else {
				wait_event_interruptible_timeout(
					ioch->inflight_discard_wait,
					atomic_read(&ioch->inflight_discard) +
					IOCACHE_INFLIGHT_DISCARD_LIMIT <=
					INFLIGHT_DISCARD_MAX_PAGES,
					ioch->threshold[idle] - cur_jiffies +
					ioch->jiffies_interval);
				if (atomic_read(&ioch->inflight_discard) +
					IOCACHE_INFLIGHT_DISCARD_LIMIT >
					INFLIGHT_DISCARD_MAX_PAGES)
					break;
...
int iocache_flush_thread_func(void *data)
{
	...
	while (1) {
		...
		ioch->jiffies_interval = jiffies - ioch->jiffies_interval;
		idle = is_idle(ioch);
		reach_limit = atomic_read(&ioch->tot_uppages) >=
			atomic_read(&ioch->iocache_page_hard_limit);
		if (ioch->jiffies_interval < ioch->threshold[idle] &&
			!reach_limit) {
			wait_event_interruptible_timeout(ioch->drain_trans_wait,
				ioch->need_drain | ioch->no_mem,
				1000 / HZ * (ioch->threshold[idle] -
				ioch->jiffies_interval));
			if (ioch->need_drain)
