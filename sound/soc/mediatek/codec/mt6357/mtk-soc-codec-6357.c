...
#if defined(CONFIG_SND_SOC_MTK_AUDIO_PA)
static const char *pa_mode_function[] = {
...
#if defined(CONFIG_SND_SOC_MTK_AUDIO_PA)
static void mtk_simple_pa_gpio_enbale_init(struct device_node *np)
{
	int ret;
	int simple_pa_gpio_enbale;
	const char *gpio_enbale = "simple-pa-gpio-enale";
	...
	simple_pa_gpio_enbale = of_get_named_gpio(np, gpio_enbale, 0);
	if (simple_pa_gpio_enbale < 0) {
		pr_info("%s: of_get_named_gpio failed, %d\n",
			__func__, simple_pa_gpio_enbale);
		ret = of_property_read_u32(np, gpio_enbale,
			(u32 *)&simple_pa_gpio_enbale);
		if (ret) {
			simple_pa_gpio_enbale = -1;
			pr_info("get simple_pa_gpio_enbale failed\n");
			return;
		}
	}

	if (simple_pa_gpio_enbale < 0) {
		pr_err("simple_pa_gpio_enbale = %d is invaild\n",
			simple_pa_gpio_enbale);
		return;
	}

	ret = gpio_request(simple_pa_gpio_enbale, "simple_pa_enale");
	if (ret) {
		pr_err("Could not request simple_pa_gpio_enbale = %d\n",
		simple_pa_gpio_enbale);
		return;
	}

	gpio_direction_output(simple_pa_gpio_enbale, GPIO_DIR_OUT);
	gpio_set_value(simple_pa_gpio_enbale, GPIO_OUT_ZERO);
	auddrv_gpio_extamp_set_gpio_num(simple_pa_gpio_enbale);
}
#endif
...
static int mtk_mt6357_codec_dev_probe(struct platform_device *pdev)
{
	...
	mtk_simple_pa_gpio_enbale_init(np);
#endif
