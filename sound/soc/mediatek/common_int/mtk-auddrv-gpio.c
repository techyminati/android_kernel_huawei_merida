...
static int g_ext_amp_enbale_gpio = -1;

void auddrv_gpio_extamp_set_gpio_num(int gpio_num)
{
	pr_info("%s g_ext_amp_enbale_gpio = %d\n", __func__, gpio_num);
	g_ext_amp_enbale_gpio = gpio_num;
}
#endif
...
#if defined(CONFIG_SND_SOC_MTK_AUDIO_PA)
// AW87318 recommended pulse delay 2us
#define PA_GPIO_DELAY     2
void auddrv_gpio_extamp_custom(int enable, int mode)
{
	int i;

	if (g_ext_amp_enbale_gpio <= 0) {
		pr_err("%s(), invalid ext emppa enable gpio = %d", __func__,
			g_ext_amp_enbale_gpio);
		return;
	}
	if (enable == 1) {
		for (i = 0; i < mode; i++) {
			gpio_set_value(g_ext_amp_enbale_gpio, GPIO_OUT_ZERO);
			udelay(PA_GPIO_DELAY);
			gpio_set_value(g_ext_amp_enbale_gpio, GPIO_OUT_ONE);
			udelay(PA_GPIO_DELAY);
		}
	} else {
		gpio_set_value(g_ext_amp_enbale_gpio, GPIO_OUT_ZERO);
...
#else
        auddrv_gpio_extamp_custom(bEnable, mode);
#endif
	mutex_unlock(&gpio_request_mutex);
#endif
	return retval;
